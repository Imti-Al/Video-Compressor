...
class VideoCompressorGUI(ctk.CTk):
    def __init__(self):
        super().__init__()
        # Essential window settings (restored from your working code)
        ...
        self.create_file_size_page()
        self.create_custom_page_frame()
    ...
    def setup_constants(self):
        # Common constants for sizing and styling.
        self.PADDING = 5
    ...
    def setup_variables(self):
        ...
        # Shared theme variable.
        self.theme_var = tk.StringVar(value="System")
        # File Size (and now Custom) page shared parameters.
        ...
        # Custom page additional parameters.
    ...
    def create_file_size_page(self):
        # File Size page.
        self.file_size_page = ctk.CTkFrame(self.main_content_frame, corner_radius=0)
        self.file_size_page.pack(fill="both", expand=True)
        ...
        # Reusable Info Section for File Size Page.
        file_size_info = self._create_info_section(self.file_size_left_frame, height=200)
        self.info_textbox = file_size_info["textbox"]
        self.progress_bar = file_size_info["progress_bar"]
        self.status_label = file_size_info["status_label"]

        # Theme segmented button for File Size Page.
        self.file_size_theme_frame = ctk.CTkFrame(self.file_size_left_frame, fg_color="transparent")
        self.file_size_theme_frame.pack(side="bottom", anchor="se", padx=self.PADDING, pady=self.PADDING)
        self.theme_segmented_button = ctk.CTkSegmentedButton(
            master=self.file_size_theme_frame,
        ...
        self.theme_segmented_button.pack(side="left")

    def create_custom_page_frame(self):
        # Build the Custom (Advanced Settings) Page.
        self.custom_page_frame = ctk.CTkFrame(self.main_content_frame, corner_radius=0)

        # Left Frame for parameters on Custom Page.
        self.custom_left_frame = ctk.CTkFrame(self.custom_page_frame, fg_color="transparent")
        self.custom_left_frame.pack(side="left", fill="both", expand=True, padx=(0, self.PADDING))
        ...
        # Parameters on Custom Page – now using the same shared variables.
        ...
        self.resolution_dropdown.grid(row=3, column=1, padx=(0, self.PADDING), sticky="w")
        self.framerate_label = ctk.CTkLabel(self.custom_input_frame, text="Framerate   ", font=self.FONT)
        self.framerate_label.grid(row=4, column=0, padx=(0,5), sticky="e")
        ...
        self.framerate_dropdown.grid(row=4, column=1, padx=(0, self.PADDING), sticky="w")
        self.bitrate_label = ctk.CTkLabel(self.custom_input_frame, text="Bitrate   ", font=self.FONT)
        self.bitrate_label.grid(row=5, column=0, padx=(0,5), sticky="e")
        ...
        self.bitrate_dropdown.grid(row=5, column=1, padx=(0, self.PADDING), sticky="w")
        self.custom_codec_label = ctk.CTkLabel(self.custom_input_frame, text="Codec   ", font=self.FONT)
        self.custom_codec_label.grid(row=6, column=0, padx=(0,5), sticky="e")
        ...
        self.custom_codec_dropdown.grid(row=6, column=1, padx=(0, self.PADDING), sticky="w")

        # Reusable Info Section for Custom Page.
        custom_info = self._create_info_section(self.custom_left_frame, height=350)
        self.custom_info_textbox = custom_info["textbox"]
        self.custom_progress_bar = custom_info["progress_bar"]
        self.custom_status_label = custom_info["status_label"]

        # Right Side – Video Info Section on Custom Page.
        self.custom_right_frame = ctk.CTkFrame(self.custom_page_frame, width=300, corner_radius=0, fg_color="transparent")
        self.custom_right_frame.pack(side="right", fill="both", expand=True, padx=self.PADDING)
        # Pack inner frame with fill="x" so it doesn't expand vertically.
        self.custom_right_inner_frame = ctk.CTkFrame(self.custom_right_frame, fg_color="transparent")
        self.custom_right_inner_frame.pack(fill="x", padx=self.PADDING, pady=self.PADDING)
        self.preview_image_label = ctk.CTkLabel(self.custom_right_inner_frame, text="", width=250, height=200)
        self.preview_image_label.pack(pady=(self.PADDING*2, self.PADDING/2))
        self.video_info_frame = ctk.CTkFrame(self.custom_right_inner_frame, fg_color="transparent")
        ...
        # Theme segmented button on Custom Page.
        self.custom_theme_frame = ctk.CTkFrame(self.custom_right_frame, fg_color="transparent")
        self.custom_theme_frame.pack(side="bottom", anchor="se", padx=self.PADDING, pady=self.PADDING)
        self.theme_segmented_button2 = ctk.CTkSegmentedButton(
            master=self.custom_theme_frame,
            values=self.THEME_OPTIONS,
            variable=self.theme_var,
            command=self.handle_theme_segmented,
            width=150,
        ...
        self.theme_segmented_button2.pack(side="left")
    ...
    def show_file_size_page(self):
        self.custom_page_frame.pack_forget()
        self.file_size_page.pack(fill="both", expand=True)

    def show_custom_page(self):
        self.file_size_page.pack_forget()
        self.custom_page_frame.pack(fill="both", expand=True)
    ...
    def browse_file(self):
        ...
        if filepath:
            ...
            # For demonstration purposes, metadata is set manually.
